...
void setup()
{
  ...
  if (mqtt_client.connect(client_id, mqtt_user, mqtt_pass))
  {
    ...
    mqtt_client.publish("TD06_GP09/temp", String(temp_measure).c_str());
    mqtt_client.publish("TD06_GP09/relhum", String(relative_humidity_measure).c_str());
  }
  else
  {
    Serial.println("Erreur connexion client broker MQTT");
  }
